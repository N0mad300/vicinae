...
std::unique_ptr<ActionPanelState> ThemeSection::actionPanel(int i) const {
  ...
  if (theme->path() && textEditor) {
    auto *open = new OpenAppAction(textEditor, "Open theme file", {QString::fromStdString(theme->path()->string())});
  ...
  if (theme->path()) {
    auto *copyPath = new CopyToClipboardAction(Clipboard::Text(QString::fromStdString(theme->path()->string())), "Copy path");

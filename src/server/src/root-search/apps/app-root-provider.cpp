...
std::unique_ptr<ActionPanelState> AppRootItem::newActionPanel(ApplicationContext *ctx,
  ...
  if (auto opener = appDb->findDefaultOpener(QString::fromStdString(m_app->path().string()))) {
    auto openLocation =
        new OpenAppAction(opener, "Open Location", {QString::fromStdString(m_app->path().string())});
